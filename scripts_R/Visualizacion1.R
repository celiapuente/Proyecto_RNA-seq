# CONTRASTE 1: wild-type flies (w1118, 1wk vs 5wk)
rm(list = ls())
...
# Cargar resultados del contraste "w1118_1wk_vs_5wk"
res_w1118_age <- read.csv("/Users/celiamp/Documents/SEMESTRE 4/bioinfo/proyecto_eve/DE_w1118_1wk_vs_5wk.csv", header = TRUE)
#row.names = 1

# ---- volcano plot ----
df <- as.data.frame(res_w1118_age)  # padj 0.05 y log2FoldChange de 2
df <- df %>%
...
png(file = paste0(figdir, "VolcanoPlot_w1118_1wk_vs_5wk.png"))
ggplot(df, aes(log2FoldChange, -log(padj, 10))) + 
  geom_point(aes(color = Expression), size = 0.7) + 
  labs(title = "w1118_1wk vs 5wk") + 
...
# --- Heatmap (vsd) ----- 
topGenes <- head(order(res_w1118_age$padj), 20)  # Obtener el nombre de los 20 genes con p value más significativo

# Visualización Heatmap con los top 20 genes
png(file = paste0(figdir, "Heatmap_vsd_topgenes_w1118_1wk_vs_5wk.png"))
...
# Obtener resultados directamente desde el objeto dds
res <- results(dds, contrast = c("condition", "w1118_1wk", "w1118_5wk"))

# Seleccionar los mismos top genes
topGenes <- head(order(res$padj), 20)

# Crear matriz de log2FC
mat <- as.matrix(res$log2FoldChange[topGenes])
rownames(mat) <- rownames(res)[topGenes]
colnames(mat) <- "condition_w1118_1wk_vs_w1118_5wk"
...
# Visualizar Heatmap del contraste
png(file = paste0(figdir, "Heatmap_log2FoldChange_topgenes_w1118_1wk_vs_5wk.png"))
pheatmap(mat,
         cluster_rows = FALSE,
         cluster_cols = FALSE,
