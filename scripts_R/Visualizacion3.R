# CONTRASTE 3 (1wk: LamCiR vs attp2)
...
# Cargar resultados del contraste "LamCiR_vs_attp2_1wk"
# Asegúrate de tener el archivo correcto para este contraste
res_LamCiR_vs_1wk <- read.csv("/Users/celiamp/Documents/SEMESTRE 4/bioinfo/proyecto_eve/DE_LamCiR_vs_attp2_1wk.csv", header = TRUE)

# ---- Volcano Plot ----
df <- as.data.frame(res_LamCiR_vs_1wk)  # padj 0.05 y log2FoldChange de 2
df <- df %>%
...
png(file = paste0(figdir, "VolcanoPlot_LamCiR_vs_1wk.png"))
ggplot(df, aes(log2FoldChange, -log(padj, 10))) +
  geom_point(aes(color = Expression), size = 0.7) +
  labs(title = "LamCiR vs attp2_1wk") +
...
# --- Heatmap (vsd) -----
topGenes <- head(order(res_LamCiR_vs_1wk$padj), 20)  # Obtener los 20 genes con p valor más significativo

# Visualización Heatmap con los top 20 genes
png(file = paste0(figdir, "Heatmap_vsd_topgenes_LamCiR_vs_1wk.png"))
...
res <- results(dds, contrast = c("condition", "LamCiR_1wk", "attp2_1wk"))

# Seleccionar top genes según padj
topGenes <- head(order(res$padj), 20)

# Crear matriz con log2FC
mat <- as.matrix(res$log2FoldChange[topGenes])
rownames(mat) <- rownames(res)[topGenes]
colnames(mat) <- "condition_LamCiR_vs_attp2_1wk"

# Aplicar límites visuales
thr <- 3
mat[mat < -thr] <- -thr
mat[mat > thr] <- thr

# Visualizar heatmap de log2FC del contraste específico
png(file = paste0(figdir, "Heatmap_log2FoldChange_topgenes_LamCiR_vs_1wk.png"))
pheatmap(mat,
         cluster_rows = FALSE,
         cluster_cols = FALSE,
