...
# --- Load data -----
# Cargar archivos
indir <- "/Users/celiamp/Documents/SEMESTRE 4/bioinfo/proyecto_eve/" # CAMBIAR !!!
outdir <- "/Users/celiamp/Documents/SEMESTRE 4/bioinfo/proyecto_eve/go_terms/" # CHECAR 
figdir <- '/Users/celiamp/Documents/SEMESTRE 4/bioinfo/proyecto_eve/go_terms/'

# ---- Analisis de terminos Go ----
# Seleccionar bases de datos
sources_db <- c("GO:BP", "KEGG", "REAC", "TF", "MIRNA", "CORUM", "HP", "HPA", "WP")

# Seleccionar un solo archivos CSV
...
# Cargar archivo
df <- read.csv(file = paste0(indir, files[1]), header = TRUE)
df$X[is.na(df$X)] <- "Missing"  # Reemplaza NA con "Missing"
rownames(df) <- df$X
head(df)
...
multi_gp <- gost(list("Upregulated" = up_genes, 
                 ...
                 organism = 'dmelanogaster') # para Drosophila es dmelanogaster
